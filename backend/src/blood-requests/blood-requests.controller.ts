...
export class BloodRequestsController {
  ...
  async createRequest(
    ...
    const request = await this.bloodRequestsService.createRequest(
  ...
  async getAllRequests(@Request() req) {
    if (req.user.role === UserRole.HOSPITAL) {
      const requests = await this.bloodRequestsService.getRequestsByHospital(
        req.user.userId,
      );
      return {
  ...
  async updateRequestStatus(
    ...
    const request = await this.bloodRequestsService.updateRequestStatus(
      id,
      status,
    );
    return {
  ...
  async createDonorBloodRequest(
    @Body() createDonorBloodRequestDto: CreateDonorBloodRequestDto,
    @Request() req,
  ) {
    const request = await this.bloodRequestsService.createDonorBloodRequest(
