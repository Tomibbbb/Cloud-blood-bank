...
export class BloodInventoryService {
  ...
  async createInventory(
    ...
    if (existingInventory) {
      throw new BadRequestException(
        'Inventory for this blood group and location already exists',
      );
    }
  ...
  async updateInventory(
    ...
    if (updateInventoryDto.expiryDate) {
      updateInventoryDto.expiryDate = new Date(
        updateInventoryDto.expiryDate,
      ) as any;
    }
  ...
  async updateStockLevel(
    ...
    if (inventoryRecords.length === 0) {
      throw new NotFoundException(
        `No inventory found for blood group ${updateStockDto.bloodGroup}`,
      );
    }

    const remainingUnits = updateStockDto.unitsToAdd;
    ...
    if (remainingUnits > 0) {
      ...
      const totalAvailable = inventoryRecords.reduce(
        (sum, record) => sum + record.unitsAvailable,
        0,
      );
